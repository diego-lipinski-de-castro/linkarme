...
const links = computed(() => {
    const _links = props.teams.links
    _links.shift()
    _links.pop()
    return _links
})
...
const destroy = (team) => {
    router.delete(route('teams.destroy', team), {
        preserveScroll: true,
        preserveState: true,
    })
}
...
<template>
    ...
    <AppSuspense><AppLayoutNew :title="$t('Teams')">
        <div class="rounded-md bg-white px-5 py-6 shadow sm:px-6">
            <div class="flex flex-col">

                <div class="flex justify-between items-center space-x-2">
                    <h2 class="text-xl font-bold leading-tight">{{ $t('Teams') }}</h2>

                    <Link :href="route('teams.create')" class="flex max-w-xs items-center rounded-md bg-blue-500 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 p-2 hover:bg-blue-700 transition-colors">
                        <span class="px-1 text-sm font-medium text-white">{{ $t('Add team') }}</span>
                    </Link>
                </div>
            </div>
        </div>
        ...
        <div class="mt-5">
            <div class="sm:hidden border border-gray-200 rounded-lg overflow-hidden">
                <ul role="list" class="divide-y divide-gray-200">
                    ...
                    <li v-for="(team, index) in teams.data" :key="index">
                        <a :href="route('teams.edit', team.id)" class="block bg-white px-4 py-4 hover:bg-gray-50">
                    ...
                    <div
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="(team, index) in teams.data" :key="index" class="bg-white">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm">
                                        <div class="flex space-x-2">
                                            <Link :href="route('teams.edit', team.id)" class="text-blue-500 hover:text-blue-700">
                                                <PencilSquareIcon class="h-5 w-5"/>
                                            </Link>

                                            <button @click="destroy(team.id)" class="text-red-500 hover:text-red-700">
