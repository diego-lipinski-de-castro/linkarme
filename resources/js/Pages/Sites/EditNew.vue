...
const submit = (confirm = false) => {
    ...
    if((unformat(form.sale, saleFormat.value) * 100) < site.suggested) {
        if(!confirm) {
            open.value = true;
            return;
        }
            ...
            <div class="fixed inset-0 z-10 overflow-y-auto">
                ...
                <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                    ...
                    <div class="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                        <button type="button" class="rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" @click="open = false">
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">{{ $t('Confirm sale value') }}</DialogTitle>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">Você informou um valor de venda menor que o valor sugerido.</p>
                        </div>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <button @click="submit(true)" type="button" class="inline-flex w-full justify-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm">{{ $t('Confirm') }}</button>
                        <button type="button" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 sm:mt-0 sm:w-auto sm:text-sm" @click="open = false">{{ $t('Cancel') }}</button>
                    </div>
                    </DialogPanel>
                </TransitionChild>
                </div>
                    ...
                    <div class="ml-12 mr-16">
                        <form @submit.prevent="submit(false)" class="grid grid-cols-1 sm:grid-cols-6 gap-x-6 gap-y-6">
                            ...
                            <div class="col-span-3">
                                ...
                                <p :class="['mt-2 ml-2 text-sm font-medium', site.positive ? 'text-green-500' : 'text-red-500']">{{ $t('Suggested') }}: {{ site.formatted_suggested }}</p>
                            ...
                            <div class="col-span-6 self-end mb-1 flex">
                                ...
                                <button v-if="site.status == 'PENDING'" @click="reject" type="button"
                                    class="mr-3 bg-red-600 border border-transparent rounded-md shadow-sm py-3 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">{{ $t('Reject') }}</button>

                                <button v-if="site.status == 'PENDING'" @click="approve" type="button"
                                    class="mr-3 bg-green-600 border border-transparent rounded-md shadow-sm py-3 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">{{ $t('Approve') }}</button>
